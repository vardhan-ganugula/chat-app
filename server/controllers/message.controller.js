...
export const getMessages = async (req, res) => {
    ...
    const loggedInUser = req.user._id;
    if(!targetUser) return res.status(404).json({ status: 'error',message: 'target user id required' });
    try {
        const messages = MessageModel.find({
            $where: {
                $or: [
                    { sender: loggedInUser, receiver: targetUser },
                    { sender: targetUser, receiver: loggedInUser }
                ]
            }
        }).sort({ createdAt: 1 });
...
export const sendMessage = async (req, res) => {
    ...
    if(!senderId || !receiverId) return res.status(404).json({ message: 'receiver id is required' });
    try {
