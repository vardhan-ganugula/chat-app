...
const ChatBox = ({ user }) => {
  const { setSelectedUser, selectedUser, allUserMessages } = useChatStore();
  ...
  return (
    ...
    >
      ...
      <div className="hidden flex-1 min-w-0 md:block">
        ...
        <p className="text-sm text-gray-500 truncate mt-0.5 space-x-4">
          <span>@{user.username.toLowerCase()}</span>
          {allUserMessages[user._id] && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 ml-2 w-10 overflow-hidden text-ellipsis">
              {allUserMessages[user._id].message}
            </span>
          )}
        </p>
      </div>
